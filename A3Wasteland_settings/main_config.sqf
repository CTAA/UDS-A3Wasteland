...
A3W_bleedingTime = 60*3;             // Time in seconds for which to allow revive after a critical injury (minimum 10 seconds)
...
A3W_serverSavingInterval = 4*60;   // Interval in seconds between automatic vehicle & object saves; should be kept at 1 min for profileNamespace and iniDB, while for extDB it can be relaxed to 3-5 mins
...
A3W_extDB_ConfigName = "db124955";      // Name of the connection config from extdb-conf.ini to be used (the one within [brackets])
...
A3W_vehicleQuantity = 220;         // Approximate number of land vehicles to be spawned in towns
...
A3W_sideMissionTimeout = 90*60;    // Time in seconds that a Side Mission will run for, unless completed
...
A3W_extraMissionDelay = 90*60;	   // Time in seconds between Extra Missions
A3W_extraMissionTimeout = 90*60;   // Time in seconds that an Extra Mission will run for, unless completed
